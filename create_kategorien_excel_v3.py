...
Version 3 - Mehrfachzuordnung: Ein Produkt kann mehreren Produkttypen zugeordnet werden
"""
...
# =============================================================================
# KATEGORIE-DEFINITIONEN MIT MEHRFACHZUORDNUNG
# =============================================================================

# Strukturierte Kategorien für die Hauptzuordnung
KATEGORIEN = {
    # =========================================================================
    # SCHLAFEN - BETTTYPEN (Mehrfachzuordnung möglich)
    # =========================================================================
    ...
    # KOSMETIK & PFLEGE
    # =========================================================================
    ('Kosmetik & Pflege', 'Körperpflege', 'Duschcreme'): [
        (r'Duschcreme', 'Duschcreme'),
    ],
    ('Kosmetik & Pflege', 'Körperpflege', 'Handcreme'): [
...
BETTGROESSEN = [
    ...
    (r'Doppelbett|140\s*x\s*200|160\s*x\s*200|180\s*x\s*200|200\s*x\s*200', 'Doppelbett'),
    (r'Kinderbett|70\s*x\s*140|60\s*x\s*120', 'Kinderbett'),
    (r'Kingsize|King.Size', 'Kingsize-Bett'),
    (r'Queensize|Queen.Size', 'Queensize-Bett'),
]

# Materialien
MATERIALIEN = [
    (r'Massivholz|Buche|Eiche|Esche|Nussbaum|Zirbe|Fichte|Ahorn', 'Massivholz'),
    (r'Polster|gepolstert|Stoffbezug', 'Polster'),
    (r'Metall|Stahl|Eisen', 'Metall'),
    (r'Leder', 'Leder'),
...
# Zielgruppen
ZIELGRUPPEN = [
    (r'Kinder|Kind|Kids', 'Kinder'),
    (r'Baby|Säugling', 'Baby'),
    (r'Damen|Frauen|Women', 'Damen'),
    (r'Herren|Männer|Men', 'Herren'),
    (r'Unisex', 'Unisex'),
]
...
def main():
    ...
    # Kategorisiere Produkte mit Mehrfachzuordnung
    produkt_kategorien = defaultdict(list)  # id -> [(hauptkat, unterkat, typ, produkttyp)]
    produkt_zusatz = defaultdict(list)  # id -> [zusätzliche Eigenschaften]

    # Schritt 1: Alle Kategorien für jedes Produkt sammeln
        ...
        for p in products:
            ...
            for pattern, produkttyp in patterns:
                if re.search(pattern, name, re.IGNORECASE):
                    ...
                    break  # Nur einmal pro Kategorie-Typ

    # Schritt 2: Zusätzliche Eigenschaften ermitteln
    for p in products:
        name = p.get('name', '').strip()
        zusatz = []

        # Bettgrößen
        for pattern, label in BETTGROESSEN:
            if re.search(pattern, name, re.IGNORECASE):
                zusatz.append(f"Größe:{label}")

        # Materialien
        for pattern, label in MATERIALIEN:
            if re.search(pattern, name, re.IGNORECASE):
                zusatz.append(f"Material:{label}")

        # Zielgruppen
        for pattern, label in ZIELGRUPPEN:
            if re.search(pattern, name, re.IGNORECASE):
                zusatz.append(f"Zielgruppe:{label}")

        # Saison
        for pattern, label in SAISON:
            if re.search(pattern, name, re.IGNORECASE):
                zusatz.append(f"Saison:{label}")

        produkt_zusatz[p['id']] = zusatz

    # Ergebnisse aufbereiten
    # Ansicht 1: Produkte mit allen zugeordneten Typen
    produkt_results = []
    for p in products:
        kategorien = produkt_kategorien[p['id']]
        zusatz = produkt_zusatz[p['id']]

        if kategorien:
            produkttypen = list(set([k[3] for k in kategorien]))
            hauptkategorien = list(set([k[0] for k in kategorien]))

            produkt_results.append({
                'Artikelnummer': p.get('product_number', ''),
                'Produktname': p.get('name', '')[:80],
                'Hauptkategorien': ' | '.join(sorted(hauptkategorien)),
                'Produkttypen': ' | '.join(sorted(produkttypen)),
                'Anzahl Typen': len(produkttypen),
                'Eigenschaften': ' | '.join(sorted(zusatz)) if zusatz else '-'
            })
        else:
            produkt_results.append({
                'Artikelnummer': p.get('product_number', ''),
                'Produktname': p.get('name', '')[:80],
                'Hauptkategorien': 'UNKATEGORISIERT',
                'Produkttypen': '-',
                'Anzahl Typen': 0,
                'Eigenschaften': ' | '.join(sorted(zusatz)) if zusatz else '-'
            })

    # Ansicht 2: Kategorie-Übersicht mit Mehrfachzuordnung
    kategorie_produkte = defaultdict(list)
    for p in products:
        for kat in produkt_kategorien[p['id']]:
            kategorie_produkte[kat].append({
                'artikelnr': p.get('product_number', ''),
                'name': p.get('name', ''),
            })

    kategorie_results = []
    for (hauptkat, unterkat, typ, produkttyp), prods in sorted(kategorie_produkte.items()):
        kategorie_results.append({
            'Hauptkategorie': hauptkat,
            'Unterkategorie': unterkat,
            'Produkttyp': typ,
            'Anzahl': len(prods),
            'Artikelnummern': ', '.join([p['artikelnr'] for p in prods]),
            'Beispiel-Produkte': ' | '.join([p['name'][:50] for p in prods[:5]])
        })

    # Unkategorisierte
    unkategorisiert = [p for p in products if not produkt_kategorien[p['id']]]
    if unkategorisiert:
        kategorie_results.append({
            'Hauptkategorie': 'UNKATEGORISIERT',
            'Unterkategorie': '-',
            'Produkttyp': '-',
            'Anzahl': len(unkategorisiert),
            'Artikelnummern': ', '.join([p.get('product_number', '') for p in unkategorisiert]),
            'Beispiel-Produkte': ' | '.join([p.get('name', '')[:50] for p in unkategorisiert[:5]])
        })

    # Statistiken für Mehrfachzuordnung
    mehrfach_count = sum(1 for p in products if len(produkt_kategorien[p['id']]) > 1)
    max_typen = max(len(produkt_kategorien[p['id']]) for p in products) if products else 0

    # DataFrames erstellen
    df_produkte = pd.DataFrame(produkt_results)
    df_kategorien = pd.DataFrame(kategorie_results)

    # Sortieren
    df_produkte = df_produkte.sort_values(['Hauptkategorien', 'Produktname'])
    df_kategorien = df_kategorien.sort_values(['Hauptkategorie', 'Unterkategorie', 'Produkttyp'])

    # Speichere als Excel mit mehreren Sheets
    output_file = 'produktkategorien_v3_mehrfach.xlsx'

    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        # Sheet 1: Kategorien-Übersicht
        df_kategorien.to_excel(writer, sheet_name='Kategorien', index=False)
        ws_kat = writer.sheets['Kategorien']
        ws_kat.column_dimensions['A'].width = 25
        ws_kat.column_dimensions['B'].width = 25
        ws_kat.column_dimensions['C'].width = 25
        ws_kat.column_dimensions['D'].width = 10
        ws_kat.column_dimensions['E'].width = 100
        ws_kat.column_dimensions['F'].width = 80

        # Sheet 2: Produkte mit Mehrfachzuordnung
        df_produkte.to_excel(writer, sheet_name='Produkte-Mehrfach', index=False)
        ws_prod = writer.sheets['Produkte-Mehrfach']
        ws_prod.column_dimensions['A'].width = 20
        ws_prod.column_dimensions['B'].width = 60
        ws_prod.column_dimensions['C'].width = 40
        ws_prod.column_dimensions['D'].width = 50
        ws_prod.column_dimensions['E'].width = 12
        ws_prod.column_dimensions['F'].width = 50

        # Sheet 3: Nur Produkte mit Mehrfachzuordnung
        df_mehrfach = df_produkte[df_produkte['Anzahl Typen'] > 1].copy()
        df_mehrfach.to_excel(writer, sheet_name='Nur-Mehrfach', index=False)
        ws_mehrfach = writer.sheets['Nur-Mehrfach']
        ws_mehrfach.column_dimensions['A'].width = 20
        ws_mehrfach.column_dimensions['B'].width = 60
        ws_mehrfach.column_dimensions['C'].width = 40
        ws_mehrfach.column_dimensions['D'].width = 50
        ws_mehrfach.column_dimensions['E'].width = 12
        ws_mehrfach.column_dimensions['F'].width = 50

    kategorisiert_count = len(products) - len(unkategorisiert)

    print(f"\nExcel-Datei erstellt: {output_file}")
    print(f"\n=== STATISTIKEN ===")
    print(f"Kategorisiert: {kategorisiert_count} von {len(products)} Produkten ({100*kategorisiert_count/len(products):.1f}%)")
    print(f"Unkategorisiert: {len(unkategorisiert)} ({100*len(unkategorisiert)/len(products):.1f}%)")
    print(f"\n=== MEHRFACHZUORDNUNG ===")
    print(f"Produkte mit mehreren Typen: {mehrfach_count} ({100*mehrfach_count/len(products):.1f}%)")
    print(f"Maximale Anzahl Typen pro Produkt: {max_typen}")

    # Zusammenfassung nach Hauptkategorie
    print("\n=== ZUSAMMENFASSUNG (ohne Mehrfachzählung) ===")
    hauptkat_count = defaultdict(set)
    for p in products:
        for kat in produkt_kategorien[p['id']]:
            hauptkat_count[kat[0]].add(p['id'])

    for kat, ids in sorted(hauptkat_count.items(), key=lambda x: -len(x[1])):
        print(f"{kat}: {len(ids)}")
